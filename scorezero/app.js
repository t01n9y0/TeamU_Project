...
const modals = {
  install(){ return `<div class="modal"><div class="sheet"><h2>加入手機主畫面</h2><div class="installSplash"><div class="splashIcon">♠</div><div><b>ScoreZero 撲克記分板</b><p>安裝後可像 App 一樣從手機桌面開啟。</p></div></div><div class="intro classic"><p>iPhone：請用 Safari 開啟，點下方分享按鈕，選<code>加入主畫面</code>。</p><p>Android：若瀏覽器跳出安裝提示，按<code>安裝</code>即可；也可從瀏覽器選單加入主畫面。</p></div><button class="secondary" style="width:100%;margin-top:12px" data-act="closeModal">知道了</button></div></div>`; },
...
function joinLobbyHtml(){
  ...
  return `<div class="field"><label>輸入房號</label><input id="joinCodeDirect" inputmode="numeric" maxlength="4" placeholder="輸入房號" value="${esc(state.joinCodeDirect||'')}"></div><button style="width:100%;margin-bottom:14px" class="secondary" data-act="joinRoom">透過房號加入</button><div class="field"><label>目前房間</label>${list}</div>${selected?`<div class="field"><label>房號密碼</label><input id="joinCode" inputmode="numeric" maxlength="4" placeholder="輸入房號" value="${esc(state.joinPreviewCode||'')}"></div><button style="width:100%;margin-bottom:14px" class="secondary" data-act="joinRoom">加入選擇房間</button>`:''}`;
}
let lobbyTimer=null;
