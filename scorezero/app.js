...
const VERSION = 'ScoreZero beta5';
const DEVICE_KEY = 'scorezero_device_id_beta5';
const LAST_ROOM_KEY = 'scorezero_last_room_beta5';
...
let unsub = [];
let state = { screen:'home', modal:null, room:null, toast:'', loading:'', createCount:3, quickDraft:{}, quickTouched:{}, light:false, awake:false, wakeLock:null, editRecId:null, joinPreview:null, joinPreviewCode:'', busy:false };
...
const views = {
  ...
  room(){
    ...
    return `<div class="topbar"><div><div class="brand smallBrand roomTitle" ${isOwner()?'data-act="editRoomName" title="點擊修改房名"':''}>${esc(r.name||'記分房')}</div><div class="sub">房號 ${r.code}　${ownerBadge}</div></div><button class="secondary small" data-act="home">首頁</button></div>
    ...
    <div class="card"><h3>快速計分</h3><div class="sub">輸入或點按快捷鍵會累加；只剩一位未手動輸入時，系統會即時自動補到總和為 0。</div><div class="quickRows">${r.players.map(p=>quickRow(p)).join('')}</div><button style="width:100%" data-act="submitQuick">提交</button><button style="width:100%;margin-top:8px" class="secondary" data-act="clearQuick">清空快速計分</button></div>
    <div class="card bottomActions"><button class="secondary" data-act="openPlayers">玩家</button><button class="secondary" data-act="openHistory">紀錄</button><button class="secondary" data-act="toggleAwake">${state.awake?'關閉常亮':'螢幕常亮'}</button><button class="secondary" data-act="toggleLight">${state.light?'關燈':'開燈'}</button>${isOwner()?'<button class="danger" data-act="closeRoom">關閉房間</button>':''}</div>`;
  }
};
...
const modals = {
  ...
  join(){ return `<div class="modal"><div class="sheet"><h2>加入房間</h2><div class="field"><label>輸入房號</label><input id="joinCode" inputmode="numeric" maxlength="4" placeholder="輸入房號" value="${esc(state.joinPreviewCode||'')}"></div><div id="roomPreview">${roomPreviewHtml()}</div><div class="field"><label>你的暱稱</label><input id="joinName" placeholder="輸入暱稱"></div><div class="grid"><button data-act="joinRoom">加入</button><button class="secondary" data-act="closeModal">取消</button></div></div></div>`; },
...
function roomPreviewHtml(){
  const p=state.joinPreview;
  if(!state.joinPreviewCode || state.joinPreviewCode.length<4) return '<div class="preview muted">輸入 4 碼後會顯示房間資訊。</div>';
  if(p==='loading') return '<div class="preview">查詢房間中...</div>';
  if(!p) return '<div class="preview muted">查無此房間。</div>';
  if(p.closed) return '<div class="preview muted">此房間已關閉。</div>';
  return `<button class="roomPreviewCard" data-act="joinRoom"><b>${esc(p.name||'記分房')}</b><span>房主：${esc(p.ownerName||'未知')}｜${Number(p.playerCount||0)} 人｜閒置 ${idleText(p.lastActiveAtMs)}</span><small>點此加入</small></button>`;
}
...
function previewRoom(v){
  const c=String(v||'').replace(/\D/g,'').slice(0,4);
  state.joinPreviewCode=c;
  state.joinPreview=null;
  const input=$('#joinCode'); if(input && input.value!==c) input.value=c;
  clearTimeout(previewTimer);
  if(c.length!==4){ render(); return; }
  state.joinPreview='loading'; render();
  previewTimer=setTimeout(async()=>{
    try{
      const snap=await getDoc(roomRef(c));
      if(!snap.exists()){ state.joinPreview=null; render(); return; }
      const data=snap.data();
      const ps=await getDocs(playersRef(c));
      state.joinPreview={...data, playerCount:ps.size}; render();
    }catch(e){ state.joinPreview=null; render(); }
  },350);
}
...
function bind(){
  ...
  const jc=$('#joinCode'); if(jc) jc.oninput=()=>previewRoom(jc.value);
